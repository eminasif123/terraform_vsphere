...
if HOST_STATUS == "no_data_exists":
 ...
 if os_type.lower() == 'Windows'.lower(): 
  ...
  dbug=os.popen('pwd').read()
  print(dbug)
  tf = Terraform(working_dir='/var/lib/jenkins/workspace/VMWARE-VM-BUILDOPS/Windows/Terraform_Code', variables={'slack_token_url':slack_token_url, 'vcenter_user':vcenter_user, 'vcenter_password':vcenter_password, 'vcenter_host':vcenter_host, 'dom_admin_user':domain_admin_user, 'dom_admin_password':domain_admin_password, 'local_admin_pass':local_admin_pass, 'host_name':host_name, 'os_template':os_template, 'dom':domain, 'vcpu':vcpu, 'mem':mem, 'dc':dc, 'clus':clus, 'ds':ds, 'nw':nw, 'host_ip':host_ip, 'netmask':netmask, 'gw':gw, 'dns1':dns1, 'dns2':dns2, 'd1':d1, 'd1typ':d1typ, 'd2':d2, 'd2typ':d2typ})
